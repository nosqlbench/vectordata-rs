...
fn cmd_completions(shell: Option<String>) {
    ...
    // protocol (the emitted bash shim calls back via `_VECD_COMPLETE`, never a
    // bare `COMPLETE=bash vecd`). With no `--shell`, auto-detect and emit the
    // indirect `source <(…)` wrapper.
    match shell {
        Some(s) => match veks_completion::Shell::from_name(&s) {
            Some(sh) => veks_completion::print_completions("vecd", sh),
            None => {
                eprintln!("unknown shell '{s}' (expected bash | zsh | fish | elvish | powershell)");
                std::process::exit(1);
            }
        },
        None => veks_completion::print_indirect_wrapper("vecd"),
    }
}
