...
fn write_dataset_yaml(
    ...
    // highest this build knows (V-4) — folded from what each facet
    // needs rather than restated here, so it cannot drift from the rule
    // (V-19). An unsharded output states version 1, which every build
    // that ever existed reads.
    let required = facets
        .iter()
        .map(|f| {
    ...
    // Stated even when it is 1 (V-25): an absent field is held to 1
    // and refused above it, so only a stated version lets a reader
