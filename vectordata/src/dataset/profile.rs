...
pub struct DSProfile {
    ...
    /// [`crate::model::ProfileConfig::attributes`]; see there for why a
    /// map rather than fields, and why absent is undescribed rather
    /// than zero (P-7).
    pub attributes: IndexMap<String, serde_yaml::Value>,
    /// The profile this one inherits unstated views from (P-2).
    ///
    /// Absent means `default`, the rule every dataset written before
    /// parameterized profiles depends on.
...
impl Serialize for DSProfile {
        ...
        S: serde::Serializer,
    {
        ...
        if !self.attributes.is_empty() { len += 1; }
        if self.inherits.is_some() { len += 1; }
        ...
        if let Some(parent) = &self.inherits {
            map.serialize_entry("inherits", parent)?;
        }
        // Emitted before the facets so a reader meets what the profile
        // *is* before what it points at. Skipped when empty: absent
        // means undescribed, and an empty map written out would make
        // "has no attributes" and "was asked and had none" look alike
        // in the file (P-7).
...
pub struct DSProfileGroup {
    ...
    /// several specs — overlapping strata membership is valid.
    ///
    /// This feeds each stratum's `series` list
    /// ([`crate::dataset::strata::Strata::sync_series`]) and marks
    /// which profiles are generated (omitted from compact `save()`
    /// output since they're re-generated from their specs on reload).
    pub series_by_spec: IndexMap<String, Vec<String>>,
}
...
impl DSProfileGroup {
    ...
    /// in generator order (P-8).
    ///
    /// `series_by_spec` has always recorded this and nothing surfaced
    /// it, so two profiles that are one corpus at two selectivities
    /// were indistinguishable from two unrelated profiles. A consumer
    /// asking "what else is this a parameterization of" had to parse
    /// names, and a name is an identifier, not a description.
    pub fn families(&self) -> impl Iterator<Item = (&str, &[String])> {
        self.series_by_spec
            .iter()
            .map(|(spec, members)| (spec.as_str(), members.as_slice()))
    }

    /// The family `name` belongs to, as `(spec, members)`.
    ///
    /// `None` for a profile no generator produced — which is a real
    /// answer, not a missing one: a hand-declared profile is a family
    /// of itself.
    pub fn family_of(&self, name: &str) -> Option<(&str, &[String])> {
        self.families()
            .find(|(_, members)| members.iter().any(|m| m == name))
    }

    /// Every member of `name`'s family paired with its attributes,
    /// in generator order (P-8).
    ///
    /// This is the shape a consumer choosing "the profile nearest 1%
    /// selectivity" needs: values, not names to parse. A member with no
    /// attributes reports an empty map — undescribed, which is not the
    /// same as a selectivity of zero (P-7).
    pub fn family_attributes(
        &self,
        name: &str,
    ) -> Vec<(String, IndexMap<String, serde_yaml::Value>)> {
        let members = match self.family_of(name) {
        ...
        members
            ...
            .filter_map(|m| {
                self.profiles
                    .get(&m)
                    .map(|p| (m.clone(), p.attributes.clone()))
            })
            .collect()
    }

    /// Returns `true` if `name` was generated by a sized/strata spec
    /// (rather than explicitly defined). Generated profiles are omitted
    /// from compact `save()` output — they're re-generated on reload.
    pub fn is_generated_profile(&self, name: &str) -> bool {
        self.series_by_spec
            .values()
            .any(|series| series.iter().any(|n| n == name))
    }
    ...
    pub fn expand_deferred_sized(&mut self, vars: &IndexMap<String, String>, base_count: u64) -> usize {
        ...
        for entry_str in &entries {
            ...
            // Record this spec's series under its original (pre-
            // interpolation) form — the same string a stratum's `spec`
            // carries — so `Strata::sync_series` can join on it. A
            // name may be recorded under several specs; overlapping
            // strata membership is valid.
            self.series_by_spec.insert(
                entry_str.clone(),
...
impl<'de> Deserialize<'de> for DSProfile {
        ...
        D: de::Deserializer<'de>,
    {
        ...
        let mut attributes: IndexMap<String, serde_yaml::Value> = IndexMap::new();
        ...
        for (key, value) in raw {
            ...
            if key == "attributes" {
                // Unknown keys are preserved, never rejected (P-5): the
                // conventional set is a convention, and a writer that
                // records something this build does not know about is
                // recording, not misbehaving. A non-map value is
                // dropped rather than fatal, for the same reason.
                attributes = serde_yaml::from_value(value).unwrap_or_default();
                continue;
            }
            if key == "inherits" {
        ...
        Ok(DSProfile { maxk, base_count, partition, views, attributes, inherits })
    }
}
...
/// it cannot (P-9).
///
/// The shard field is four digits and always last before the
/// extension, so an all-digit name interpolated ahead of it produces
/// `…__0010__0000.ivecs` — which reads as shard 10 of one facet or
/// shard 0 of another, with no way to choose
/// (`srd-multifile-facet-shards.md`, SH-101).
///
/// **Conditioned on the template actually being sharded.** An
/// all-digit profile name is not ambiguous by itself, and rejecting it
/// outright would refuse strata specs that have always generated names
/// like `5` into unsharded paths (P-13). The ambiguity is a property of
/// the pair, so the pair is what is checked. SH-101 catches the same
/// fault at realization; this catches it at the point the name is
/// invented, before there are files on disk carrying it.
fn interpolation_objection(name: &str, facet_key: &str, template: &str) -> Option<String> {
    if !template.contains(crate::dataset::shards::SHARD_FIELD) {
        return None;
    }
    ...
    Some(format!(
        "generated profile name '{name}' is all digits and facet '{facet_key}' \
         interpolates it into a sharded filename ('{template}'), which is \
         ambiguous: '…__{name}__0000.ext' has two readings and neither is \
         decidable (P-9, SH-101). Give the spec names with a non-numeric part."
    ))
}

/// Fill each profile that names a parent with the parent's views it/// Fill each profile that names a parent with the parent's views it
/// does not already have (P-2, P-3).
///
/// Default-inheritance runs first and withholds the per-profile
/// outputs, because on a **size** axis they cannot be shared: ground
/// truth is derived from `base_count`, so a 1m profile inheriting the
/// default's full-base neighbours would mis-route every verify. On a
/// **selectivity** axis every member has one `base_count` and the
/// unfiltered ground truth is invariant, so withholding it forces each
/// member to restate the same path — a duplication that drifts the
/// moment one is edited.
///
/// Naming a parent is the statement that decides which axis this is.
/// Nothing names one unless it was written to, so every dataset
/// predating this inherits exactly as before (P-13).
///
/// Parents are resolved before their children so a chain
/// (`1m-sel001` → `1m` → `default`) settles in one pass. A cycle, or a
/// parent that does not exist, leaves the profile with what it
/// declared — the same outcome as naming no parent — and is reported by
/// `veks check` rather than failing the load, because the facets it
/// does declare are still readable.
fn apply_named_parent_inheritance(profiles: &mut IndexMap<String, DSProfile>) {
    ...
    for name in &names {
        if profiles.get(name).is_some_and(|p| p.inherits.is_none()) {
    ...
    for _ in 0..names.len() {
        ...
        for name in &names {
            ...
            let Some(parent_name) = profiles.get(name).and_then(|p| p.inherits.clone()) else {
                continue;
            };
            if parent_name == *name || !settled.contains(&parent_name) {
                continue;
            }
            let Some(parent) = profiles.get(&parent_name).cloned() else {
                // A parent that is not a profile at all: nothing to
                // inherit, and nothing to wait for.
                settled.insert(name.clone());
                progressed = true;
                continue;
            };
            if let Some(child) = profiles.get_mut(name) {
                for (key, view) in &parent.views {
                    if !child.views.contains_key(key) {
                        child.views.insert(key.clone(), view.clone());
                    }
                }
                if child.maxk.is_none() {
                    child.maxk = parent.maxk;
                }
                if child.base_count.is_none() {
                    child.base_count = parent.base_count;
                }
            }
            settled.insert(name.clone());
            progressed = true;
...
/// View keys whose value is per-profile and must NOT be inherited from/// View keys whose value is per-profile and must NOT be inherited from
...
/// Parse a sized entry against a known base-count bound. Implicit
/// upper-bound forms (`mul:start/factor`, `fib:start`, `decade`)
/// resolve their end against `max_count`, and every generator stops
/// producing candidates below it — a profile at or above `max_count`
/// is redundant with the default profile.
pub fn parse_sized_entry_bounded(entry: &str, max_count: u64) -> Result<Vec<(String, u64)>, String> {
    parse_sized_entry_impl(entry, max_count)
}
...
fn parse_fib_range(range_str: &str, max_count: u64) -> Result<Vec<(String, u64)>, String> {
    ...
    loop {
        ...
        if val >= start && (max_count == 0 || val < max_count)
            // Deduplicate: fib seeds (1, 1) emit the start value twice.
            && result.last().is_none_or(|&(_, prev): &(String, u64)| prev != val)
        {
            result.push((format_count_with_suffix(val), val));
        }
...
impl<'de> Deserialize<'de> for DSProfileGroup {
        ...
        D: de::Deserializer<'de>,
    {
        ...
        let mut series_by_spec: IndexMap<String, Vec<String>> = IndexMap::new();
        // Names actually inserted by sized expansion (as opposed to
        // merely recorded in a spec's series) — used to tell a benign
        // multi-strata overlap apart from a collision with an
        // explicitly defined profile.
        ...
        for (name, value) in &raw {
                ...
                for entry_str in &entries {
                    if entry_str.contains("${") || needs_base_count(entry_str) {
                        ...
                        let pairs = parse_sized_entry(entry_str)
                            .map_err(de::Error::custom)?;
                        // Record this spec's series so strata can list
                        // the profile names each generator produced.
                        series_by_spec.insert(
                            entry_str.clone(),
                            pairs.iter().map(|(n, _)| n.clone()).collect(),
                        );
                ...
                for (prof_name, count) in all_pairs {
                    ...
                    let scaffold_views = if let Some(ref templates) = facet_templates {
                        ...
                        for (facet_key, template) in templates {
                            if let Some(reason) =
                                interpolation_objection(&prof_name, facet_key, template)
                            {
                    ...
                    if profiles.contains_key(&prof_name) {
                        if generated_names.contains(&prof_name) {
                            ...
                            // spec — a profile may belong to multiple
                            // strata. The name encodes the count, so
                            // the generated profiles are identical;
                            // keep the first.
                            continue;
                        }
                    ...
                    generated_names.insert(prof_name.clone());
            ...
            let merged = if is_self_contained {
                ...
                DSProfile {
                    ...
                    views: child.views,
                    // What the child said about *itself* survives the
                    // merge: a merge combines views, and these describe
                    // the profile rather than what it points at.
                ...
                let child_inherits = child.inherits;
        ...
        // Resolve profiles that name a parent other than `default`.
        // The loop above has already applied default-inheritance, so
        // what remains missing on such a profile is exactly what the
        // size axis withholds — the per-profile outputs — which is
        // what a family varying along a *different* axis needs to
        // share (P-2).
        apply_named_parent_inheritance(&mut profiles);
...
"#;
    ...
    fn test_fib_range() {
        ...
        // deduplicated start (the seed pair emits 1 twice): 1m, 2m, 3m,
        // 5m, 8m, 13m, 21m, 34m, 55m, 89m, 144m, 233m, 377m. The series
        // feeds strata `series:` lists, so members must be unique.
        assert_eq!(counts[0], 1_000_000);
        assert_eq!(counts[1], 2_000_000);
        assert_eq!(counts[2], 3_000_000);
        assert_eq!(counts[3], 5_000_000);
        assert_eq!(counts[4], 8_000_000);
