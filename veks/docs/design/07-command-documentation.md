...
### REQ-DOC-05: `help` subcommand and `--help` rendering

Documentation is surfaced through multiple paths:

1. **`veks help <command-path>`** — Renders the full markdown `body` to
   the terminal. Uses a lightweight markdown renderer (e.g., termimad or
   similar) for syntax highlighting and formatting.

2. **`veks pipeline <group> <command> --help`** — Same as above, rendered
   through clap's help system with the `summary` as the `about` text and
   the `body` as `long_about`.

3. **`veks help --list`** — Lists all commands with their summaries,
   grouped by category.

4. **`veks help --markdown <command-path>`** — Emits raw markdown to
...
| REQ-DOC-05 | **Done** | `veks help <command-path>`, `--list`, `--markdown` modes in main.rs |
