...
/// Gets the Content-Length of a remote resource via HEAD request.
/// Returns `None` if the server doesn't provide it or the request fails.
...
/// Downloads a file from `url` to `dest` using libcurl.
