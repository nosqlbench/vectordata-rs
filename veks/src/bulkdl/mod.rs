...
pub async fn run(args: BulkDlArgs) {
    ...
    let config_text = fs::read_to_string(&args.config).unwrap_or_else(|e| {
        ui.emitln(format!("Error reading config file {}: {}", args.config.display(), e));
        std::process::exit(1);
    });

    let config: Config = serde_yaml::from_str(&config_text).unwrap_or_else(|e| {
        ui.emitln(format!("Error parsing config: {}", e));
        std::process::exit(1);
    });
    ...
    for dataset in &config.datasets {
        ...
        fs::create_dir_all(&dataset.savedir).unwrap_or_else(|e| {
            ui.emitln(format!("Failed to create directory {}: {}", dataset.savedir, e));
            std::process::exit(1);
        });
        ...
        let pb = ui.bar(total as u64, &dataset.name);
        pb.set_position(already_done as u64);

        if pending.is_empty() {
            pb.set_message(format!("{} (complete)", dataset.name));
            pb.finish();
            continue;
        }

        // Capture the progress id for cross-thread sharing (ProgressId is Copy)
        let pb_id = pb.id();
        ...
        pb.set_message(format!("{} (done)", dataset.name));
        pb.finish();
    }
