...
//! CLI shell for the `veks` binary — spec assembly, completion dispatch,
//! help/version handling, and command dispatch.
//!
//! Lives in the library (rather than `main.rs`) so wrapper binaries — the
//! workspace-root `vectordata` multiplexer in particular — can embed the
//! identical personality; `veks/src/main.rs` is a thin shim over
//! [`bin_main`].

#![allow(dead_code)] // legacy completion functions pending removal

use crate::{cli, datasets, pipeline, prepare};
...
/// name). Wrapper binaries pass their own tail; `veks/src/main.rs` passes
/// `std::env::args().skip(1)`.
pub fn bin_main(raw: Vec<String>) {
    ...
    if raw.iter().any(|a| a == "---audit-options") {
...
fn render_markdown_to_terminal(body: &str) {
    ...
    let use_ansi = crate::term::use_color();
