...
| `--rerun STEP[,STEP...]` | Run the named steps again whether or not they look fresh; an entry may be a pattern with `*`, so `compute-postfiltered-knn*` names the step of every profile. their records are set aside for this run, and every step that declares them as an upstream follows, as does anything whose input they rewrite. For a build that changed what a step computes without changing any option, when `--provenance strict` would re-run the whole ladder. A name that matches no step is an error. |
